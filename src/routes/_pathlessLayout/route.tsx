...
function PathlessLayoutComponent() {
  ...
  return (
    <div className="flex max-h-screen max-sm:px-2 p-4">
      {/* Right Section: Carousel */}
      <div className="hidden rounded--l-lg h-full overflow-hidden md:block md:w-1/2 lg:w-4/9 relative">
        <Carousel
          showThumbs={false}
          showStatus={false}
          showArrows={false}
          ...
          {carouselItems.map((item, index) => (
            <div key={index} className="h-[calc(100vh-40px)] relative text-white">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12 pb-20 z-10"> {/* Added pb-20 to lift text above dots */}
                <div
                  key={showTextIndex} // Use key to force re-render/animation
      ...
      <div className="flex flex-1 max-md:min-h-[calc(100vh-40px)]  ">
        <div className="w-full gap-2 overflow-hidden flex h-full p-8 bg-white  relative rounded-r-lg shadow-md">
          {/* <span className="h-full bg-main w-[2px] right-4 relative opacity-20"></span> */}
